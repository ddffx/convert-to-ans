...
const _getExternalGameObj = (el) => {

    let $ = cheerio.load(el);
    let $el = $(el);

    let embed_src = $(el).data('embed-src');
...
exports.parse = elem => {
    ...
    } else if (elem.attribs.class && elem.attribs.class.match(/external-game-embed/)) {
        return _getExternalGameObj(elem);
    } else {
